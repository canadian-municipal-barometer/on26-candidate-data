...
- `notes/election-type-ward-type.csv` (one row per municipality) and `notes/municipal-election-structure.csv` (one row per race, with the municipality's attributes repeated across its rows) are generated by `R/get-municipal-data.R` — do not hand-edit either.

`R/get-municipal-data.R`: Rebuilds both generated files above from the master municipality list and the study's municipality Google Sheet, applying the corrections in `notes/classification-overrides.csv` and deriving ballot structure from `notes/council-races.csv`.
