...
- Prints any race whose seat count is unverified upstream. Acclamation cannot be computed for those, so `accl` is null and the survey leaves the field blank rather than guessing. There are none as the data stands: Kitchener's and Cambridge's at-large regional races were the only two, and both are now in `notes/excluded-races.csv`. The check stays as a general guard, and `meta.unverified_seats` is written only when it fires.
